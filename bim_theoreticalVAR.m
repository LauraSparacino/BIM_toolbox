...
function [Am,Su,Ak,z]=bim_theoreticalVAR(M,par)

% M: number of processes
% par.poles{m}=([rho f]); % Autonomous oscillations in process m (rho:
% radius of the pole, f: frequency of the pole)
% par.coup=[i j k c; ...]; % in each row: "i j k c" to impose coupling from i to j at lag k with coeff c 
